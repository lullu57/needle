...
```sh
git clone https://github.com/lullu57/needle.git
cd needle
npm ci
cp .env.example .env
```
...
2. Choose **Load unpacked** and select **`needle/extension/`** folder. This is the folder containing `manifest.json`. Do not select the repository root or a ZIP file.
...
1. Import your fork into Vercel. Use the repository root as **Root Directory** and Framework Preset to **Vite**. The included `vercel.json` specifies the build output and API function duration.
...
This standalone repository is derived from [Needle in Shubhamsaboo/awesome-llm-apps](https://github.com/Shubhamsaboo/awesome-llm-apps/tree/main/advanced_llm_apps/needle). Local changes require authenticated searches and explicit extension search actions.
